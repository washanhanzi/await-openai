...
use super::{
    chat_completion_object::{
        Choice as ChatCompletionChoice, Logprobs, Message, Response as ChatCompletionResponse,
        Role, Usage,
    },
...
impl EventDataParser<Chunk, Chunk, ChatCompletionResponse> for OpenaiEventDataParser {
    ...
    fn parse_new_tool_call(
        ...
        match (self.tool_call.take(), new_tool_call) {
            ...
            (Some(mut prev_tool_call), Some(new_tool_call)) => {
                // If we see a new tool call with different id or index, complete the previous one
                // and start fresh with the new one
                if (new_tool_call.id.is_some() && prev_tool_call.id != new_tool_call.id)
                    || prev_tool_call.index != new_tool_call.index
                {
